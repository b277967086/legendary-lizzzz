package com.good.diaodiaode.tebiediao.config;


import com.good.diaodiaode.tebiediao.model.Type;

/**
 * Created by jzxiang on 16/5/15.
 */
public class DefaultConfig {
    public static final Type TYPE = Type.ALL;
    public static final int COLOR = 0xFFFFFFFF;//0XFFE60012;
    public static final int TOOLBAR_TV_COLOR = 0xFFFFFFFF;
    public static final int TV_NORMAL_COLOR = 0xFF878787;
    public static final int TV_SELECTOR_COLOR = 0XFF6281C2;
    public static final float ITEM_HEIGHT = 33.6f;
    public static final int TV_SIZE = 12;
    public static final int TV_SELECTED_SIZE = 14;
    public static final boolean CYCLIC = true;
    public static String CANCEL = "取消";
    public static String SURE = "确定";
    public static String TITLE = "";
    public static String YEAR = "年";
    public static String MONTH = "月";
    public static String DAY = "日";
    public static String HOUR = "时";
    public static String MINUTE = "分";
    public static String AMPM = "午";


}
